...
list_of_states[[ii]]$target_end_date <- as.Date(list_of_states[[ii]]$target_end_date)
...
```{r}
# 2) build your forecast_df for i=55, ii=8

i  <- 57 # target_end_date
ii <- 8 # state number
...
conceptual_figure = ggplot() +
  # 3a) two-year observed history as a black line (mapped to color for legend)
  geom_point(data = subset_data, 
            aes(x = target_end_date, y = cases, color = "Fitting data"), 
            size = 2.5) +
  
  # 3b) forecast ribbon (not in legend)
  geom_ribbon(data = forecast_df, 
              aes(x = Dates, ymin = Lower, ymax = Upper), 
              fill = "gray", alpha = 0.4) +
  
  # 3c) forecast mean (dashed blue line + hollow points)
  ...
  # 3d) actual post-cutoff cases (mapped to color for legend)
  geom_point(data = forecast_df, 
...
#custom_patterns <- custom_patterns[custom_patterns != "circle"]
...
plot1 <- ggplot(W1, aes(x = Best_Result,
  ...
  geom_bar_pattern(
    ...
    pattern_fill  = "black",
    pattern_size  = 0.2,
    pattern_angle = 45             # set globally, not inside aes()
  ) +
  scale_fill_manual(values   = custom_colors) +
  ...
  guides(pattern_angle = "none")  # this line is optional now but safe
...
combined_plot <- ggplot(all_weeks, aes(x = Best_Result, 
  ...
  geom_bar_pattern(
    pattern_fill  = "black",
    pattern_size  = 0.1,
    pattern_angle = 45,  # set globally
    colour        = NA
  ) +
  scale_fill_manual(values = custom_colors) +
  scale_pattern_manual(values = custom_patterns) +  # define this before
 ...
 # guides(pattern_angle = "none")  # just in case
...
custom_fills <- c("AUTO" = "white", "ES27" = "white", "ES64" = "white")  # or any colors you like
...
# Define fill colors for approaches
...
ggsave("Fig4.jpg", final_plot, width = 10, height = 6)
```
