...
for (week in weeks) {
  ...
  # Step 1: Calculate % difference at state level
  data__ <- data__ %>%
    mutate(Diff_state = ((AUTO_AVG_LB / AUTO_AR) - 1) * 100)

  # Step 2: Aggregate sum of components across states for mean difference
  summed_data <- data__ %>%
    group_by(Julian_date) %>%
    summarize(
      AUTO_AR = sum(AUTO_AR, na.rm = TRUE),
      AUTO_AVG_LB = sum(AUTO_AVG_LB, na.rm = TRUE),
      SD_Diff = sd(Diff_state, na.rm = TRUE)  # SD of raw state-level % differences
    ) %>%
    mutate(
      Difference = ((AUTO_AVG_LB / AUTO_AR) - 1) * 100,  # Mean from totals
      Week = paste0("Week ", week)
    )
...
model_improv <- ggplot(summed_data_all, aes(x = Julian_date, y = Difference, color = Week)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = Difference - SD_Diff, ymax = Difference + SD_Diff), width = 0.5) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed", size = 0.8) +
  theme_minimal() +
  labs(
    title = "Figure 10: Mean percentage of WIS differences across all states",
