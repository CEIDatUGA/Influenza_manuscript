...
plt0 <- ggplot(df_combined_, aes(x = Julian_date, y = WIS, group = STATE)) + 
  ...
  theme_minimal() +  
  labs(title = "Figure 2: Baseline model WIS for 48 states on the contiguous U.S.",#,
...
combined_plot <- (plt1 | plt2) + 
  plot_annotation(
    title = "Figure 3: Mean WIS for our 24 models",
...
# Save to file
ggsave("Fig3.jpg", combined_plot, width =13, height =7)
```
...
combined_plot <- ggplot(all_weeks, aes(x = Best_Result, fill = Best_Result)) +
  geom_bar() +
  scale_fill_manual(values = custom_colors) +
  labs(title = "Figure 4: Best model based on the lowest mean WIS",
...
plot_m_heatmap <- ggplot(p_df, aes(x = factor(WeekAhead), y = Model, fill = m)) +
  ...
  labs(title = "Figure 5: Mean differences between models' WIS and the baseline across 48 states",
...
combined_plot <- (ES_1WEEK | ES_2WEEK) /
                 (ES_3WEEK | ES_4WEEK) + 
  plot_annotation(
    title = "Figure 6: Mean WIS for the 48 states in the contiguous U.S.",
...
reg2<-ggplot(regression_models, aes(x = Week_Ahead, y = independent_variable, fill = significance)) +  
  ...
  labs(title = "Figure 7: Regression models",
...
ggsave(reg2, height = 6, width = 8, filename="Fig7.jpg")  
...
ES_1WEEK <- ggplot(W1_map) +
  ...
  geom_sf_text(data = W1_map, aes(label = round(percentage_improvement,0)),  # Round to whole numbers
               ...
               check_overlap = TRUE,fontface = "bold") +  # Display percentage improvement in each state
  labs(
    fill = "Percentage (%)",  # Label for the legend
       x = "",
       y = ""
  )  # Adding a subtitle
...
combined_plot <- (ES_1WEEK | ES_2WEEK) /
                 (ES_3WEEK | ES_4WEEK) + 
  plot_annotation(
    title = "Figure 8: Mean WIS differences (%) compared to the baseline",
...
reg2<-ggplot(regression_models, aes(x = Week_Ahead, y = independent_variable, fill = significance)) +  
  ...
  geom_text(aes(label = round(r_squared, 3)), color = "black", size = 6) +  # Text inside boxes
  scale_fill_manual(values = c("Significant" = "cyan", "Not Significant" = "gray")) +  
  labs(title = "Figure 9: Regression models",
...
model_improv<-ggplot(summed_data_all, aes(x = Julian_date, y = (Difference), color = Week)) +
  ...
  theme_minimal() +
  labs(
    title = "Figure 10: Mean percentage of WIS differences across all states",
...
```
Talvez incluir standar deviation das diferenceas
...
for (week in weeks) {
  ...
  # Calculate the percentage difference at the row level (i.e., per state-date)
  data__ <- data__ %>%
    mutate(Difference = ((AUTO_AVG_LB / AUTO_AR) - 1) * 100)

  # Group by Julian date and calculate mean and standard deviation of Difference
  summarized <- data__ %>%
    group_by(Julian_date) %>%
    summarize(
  ...
  summed_data_list[[week]] <- summarized
}

# Combine all weeks into a single data frame
summed_data_all <- bind_rows(summed_data_list)

# Plot the mean difference with error bars for standard deviation
model_improv <- ggplot(summed_data_all, aes(x = Julian_date, y = Mean_Diff, color = Week)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = Mean_Diff - SD_Diff, ymax = Mean_Diff + SD_Diff), width = 0.5) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed", size = 0.8) +
  theme_minimal() +
  labs(
    title = "Figure 10: Mean percentage of WIS differences across all states",
...
#ggsave(model_improv, height = 4.5, width = 9, filename = "Fig11.jpg")
