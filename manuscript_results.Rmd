...
plot_m_heatmap <- ggplot(p_df, aes(x = factor(WeekAhead), y = Model, fill = m)) +
  ...
  labs(title = "Mean model improvement and standard deviation comparing each model' WIS with the baseline across 48 states",
...
ggplot(summed_data_all, aes(x = Julian_date, y = (Difference)*100, color = Week)) +
#  geom_line(size = 1) +        # Line plot
  geom_point(size = 2) +       # Points on the line
#  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 1) +  # Reference line at 0
  theme_minimal() +
  labs(
    title = "Percentage of WIS difference across all states AUTO_AVG_LB compared to baseline",
    x = "Julian Date",
    y = "Difference (1 - AUTO_AR_NoLg / AUTO_AVG)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```